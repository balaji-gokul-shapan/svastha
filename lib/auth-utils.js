...
// import { refreshAccessToken as refreshAccessTokenAction } from "../lib/features/auth-slice";
...
export async function refreshAccessToken() {
  ...
  if (!refreshToken) {
    console.warn("No refresh token available in sessionStorage");
    return null;
  }
  ...
  refreshInProgress = (async () => {
    try {
      ...
      if (!response.ok) {
        // Refresh token is invalid/expired — clear session so user is redirected to login
      ...
      // Update Redux store via store subscribe (sessionStorage sync)
      // The store.subscribe handler in store.js will pick up the change
      // when we dispatch the auth/refreshAccessToken action.

      // Schedule the next proactive refresh.
      scheduleProactiveRefresh(60, null);
  ...
  if (isAccessTokenExpired()) {
    ...
    if (session) {
      ...
      if (dispatch) {
        ...
        // dispatch(refreshAccessTokenAction(session));
    ...
    // Token is still valid — ensure proactive refresh is scheduled.
    scheduleProactiveRefresh(60, dispatch);
  }
