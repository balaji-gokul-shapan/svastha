...
//               ""
//             : "";
//         console.log(rawRefreshToken, refreshTokenValue,"refreshTokenValue");
...
export const loginUser = createAsyncThunk(
  ...
  async ({ username = "", password = "" } = {}, { rejectWithValue }) => {
    ...
    try {
      ...
      // ============================================================
      // Backend returns expires_in in seconds (e.g., 3600 for 1 hour).
      // We use this to calculate the exact expiry timestamp.
      // ============================================================

      const expiresIn = responseData?.expires_in ?? 3600; // Default to 1 hour if not provided
      const expiresAt = Date.now() + expiresIn * 1000;

      console.log("Token expires_in:", expiresIn, "seconds");
      ...
      return {
        ...
        username:
          staff?.user_name || normalizedUsername,
        ...
        // Exact timestamp used by the frontend
        expires_at: expiresAt,

        user: staff,

        // Account object returned by the login API (`data.data.account`).
