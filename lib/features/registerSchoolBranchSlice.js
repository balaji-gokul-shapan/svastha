...
export const getSchoolBranch = createAsyncThunk(
  ...
  async (_, { rejectWithValue, dispatch }) => {
    try {
      ...
      // Tolerate both a wrapped { data: record } payload and a raw record —
      // some roles/response versions omit the `data` wrapper. Consumers
      // already read `record?.data ?? record`, so returning the record
      // directly is safe.
