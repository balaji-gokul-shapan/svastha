  ...
  if (!token && typeof window !== "undefined") {
    try {
      const session = JSON.parse(
        window.sessionStorage.getItem("svastha-auth"),
      );
      token = session?.token;
...
export const registerStudent = createAsyncThunk(
  ...
  async ({ student: values, profileImage } = {}, { rejectWithValue, getState }) => {
    try {
      ...
      if (profileImage instanceof File) {
        ...
        fetchOptions = { method: "POST", headers, body: formData };
      } else {
        fetchOptions = {
          method: "POST",
      ...
      const response = await fetch("/api/students", fetchOptions);
