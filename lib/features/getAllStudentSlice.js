...
export const getAllStudent = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      // Branch filter. The students page passes the selected branch id as
      // `schoolName` (the URL `school` value). The backend's /students/filter
      // endpoint expects the branch ID under the `branch_id` query param.
      if (schoolName && schoolName !== "all") {
        params.set("branch_id", schoolName);
      }
