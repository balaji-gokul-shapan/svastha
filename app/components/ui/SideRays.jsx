...
const SideRays = ({
  ...
  useEffect(() => {
    ...
    // Prevent duplicate/racing initializations (React StrictMode remounts).
...
void main() {
  ...
  gl_FragColor = color;
      ...
      // Recover whenever the container's layout settles late (first paint,
      // font loading, redirect-driven reflows) — guarantees the rays show up
      // even if the very first measurement was 0x0.
      ...
      const loop = t => {
        ...
        try {
          ...
          // A single bad frame must not kill the animation forever.
          failures += 1;
          if (failures < 30) {
            animationIdRef.current = requestAnimationFrame(loop);
          }
