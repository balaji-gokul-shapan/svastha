...
function ToothButton({ tooth, isSelected, onSelect }) {
  ...
  return (
    <button
      ...
      className={cn(
        "flex size-4 md:size-7 lg:size-6 xl:size-9 shrink-0 items-center justify-center rounded-md transition-colors",
  ...
  return (
    <div className="flex items-center justify-center  gap-0.5 px-1 my-4">
      {teeth.map((number) => (
        <span
          key={number}
          className="flex shrink-0 items-center justify-center text-[10px] font-medium text-muted-foreground size-4 md:size-9 lg:size-6 xl:size-9  sm:text-[11px]"
        >
          {number}
        </span>
      ))}
...
export function ToothChartSvg({ chart, selectedTooth, onSelectTooth }) {
